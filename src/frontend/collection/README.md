# Frontend Collection Architecture

`src/frontend/collection` has one public categorization level.

Collection means collection. In Murali, it is the author-facing store of reusable visual tattvas and
teaching components.

There are three kinds of folders here:

- implementation families such as `ai`, `maths`, `text`, `primitives`, `layout`,
...
naturally search by topic. The crate root also exposes this module as `murali::collection`.
...
- `maths`: math-facing notation, graphing, and quantitative teaching domains.
...
`calculus`, `optimization`, `information_theory`, and `data_geometry` live under `maths`.
