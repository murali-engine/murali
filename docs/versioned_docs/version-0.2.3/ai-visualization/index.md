...
Murali's sangrah is the author-facing collection of reusable visual tattvas. Its implementation
families remain organized by primitive family: `ai`, `math`, `graph`, `text`, `layout`,
`storytelling`, and related modules. Alongside those, Murali exposes domain-oriented category
modules directly under `sangrah`:

```rust
use murali::sangrah::ai::transformers_llms::*;
use murali::sangrah::ganit::probability::*;
use murali::sangrah::ganit::data_geometry::scatter_plot::ScatterPlot;
```
...
Support pieces stay in implementation families such as `primitives`, `text`, `math`, `graph`,
`composite`, `layout`, `storytelling`, `table`, and `utility`. Domain facades are kept only where
they clarify the teaching subject.

## Source Architecture

The source-visible category READMEs live in
[`src/frontend/sangrah`](https://github.com/ravishankarkumar/murali/tree/main/src/frontend/sangrah).
