...
## Sangrah Category Coverage

Every newly added `sangrah` category has at least one exported component and at least one runnable
example anchor:

| Category | Example anchor |
| --- | --- |
| `ganit::notation` | `equation_and_matrix_animation`, `latex_and_typst` |
| `ganit::graph` | `graphs_2d` |
| `ganit::basic_math` | `equation_and_matrix_animation` |
| `ganit::linear_algebra` | `linear_algebra_vectors`, `linear_algebra_matrix_vector` |
| `ganit::calculus` | `graphs_2d`, `curves_3d`, `traced_paths` |
| `ganit::probability` | `next_token_distribution` |
| `ganit::statistics` | `normalization`, `graphs_2d` |
| `ganit::optimization` | `force_fields`, `traced_paths`, `surfaces_3d` |
| `ganit::information_theory` | `next_token_distribution`, `transformer_attention` |
| `ganit::data_geometry` | `graphs_2d`, `tensor_slicing`, `linear_algebra_basis_change` |
...
These examples use `murali::frontend::sangrah::ganit::linear_algebra`, which is currently gated
