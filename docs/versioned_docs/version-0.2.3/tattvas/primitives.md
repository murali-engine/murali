...
- `width` - Width in world units
- `height` - Height in world units
- `radius` - Corner radius in world units
- `color` - Fill color (RGBA)

**Best for:** Concept cards, UI panels, callouts, and diagram nodes.

**Animation pairing:** Use `.appear()`, `.write()`, or `.scale_to()`.

### ChatBubble

A filled chat/message bubble with rounded corners and a single continuous boundary around its
bottom tip.
...
ChatBubble::new(width: f32, height: f32, radius: f32, color: Vec4)
    .with_tip(ChatBubbleTipSide::Right, tip_width, tip_height)
    .with_tip_inset(0.72)
    .with_stroke(0.018, stroke_color)
```

**Parameters:**
- `width` - Bubble body width in world units
- `height` - Bubble body height in world units
- `radius` - Corner radius in world units
- `color` - Fill color (RGBA)
- `tip_width` - Width of the bottom tip where it joins the body
- `tip_height` - Vertical depth of the bottom tip
- `tip_inset` - Distance from the selected side to the tip center

**Best for:** Chat bubbles, prompt-entry scenes, dialogue moments, and explainer UI.

**Animation pairing:** Use `.appear()`, `.fade_to()`, or `.scale_to()`. For a full prompt-entry
component with typewriter text, use the beta [`ChatInputBox`](../beta/chat-input-box.md).
...
scene.set_scale(apple_id, Vec3::splat(1.2));
```

`Prop3D` is meant for video objects such as apples, balls, books, animals, tools, and simple
teaching props. It loads static geometry, base-color factors, and common embedded base-color
textures from GLB files and loose glTF folders. Prefer `.glb` for portable single-file assets; use
`.gltf` only when its `.bin` and texture files stay beside it. It does not currently load skeletal
animation, physics metadata, advanced PBR lighting, or engine-specific materials.

**Best for:** Educational video props, object examples, simple 3D scene dressing, and child-friendly
alphabet or concept videos.

**Animation pairing:** Use `.move_to()`, `.rotate_to()`, `.scale_to()`, `.fade_to()`, or composed
motion segments for bounce/jump effects.

For free asset sources and troubleshooting, see [3D Prop Assets](../3d-prop-assets.md).
...
| Chat/message bubble | ChatBubble | Single continuous bubble outline with configurable bottom tip |
