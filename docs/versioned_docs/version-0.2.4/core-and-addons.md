...
The current Rust crate can remain `murali` while the product name becomes **Murali Engine**. Over
time, the repository can grow into a workspace:
...
If the `murali` name is available on PyPI, it can be a convenience metapackage later. The first
Python release should prefer an explicit name such as `murali-engine`.
...
The exact internal module split can evolve inside `murali-kit`. Separate PyPI packages can wait
...
Premium add-ons should be separate packages, not hidden branches inside the core engine.
...
Start with `murali-engine` for Python:
...
The first Python release should expose the core engine surface: create a scene, add common visual
...
2. Build the first `murali-engine` PyPI release around the core authoring surface.
